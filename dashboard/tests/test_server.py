...
# monitoring/provider_usage.py -- tests/test_provider_usage.py owns them,
# pointed at that module. The classifiers are now typed, so the interesting
# tests there are the payloads they refuse.
...
# ── SSH / Tailscale connection checks ───────────────────────────────────────
# The roster, both probes, the debounce and the log tail moved to
# monitoring/ssh_checks.py -- tests/test_ssh_checks.py owns them, pointed at
# that module. That is not cosmetic: `monkeypatch.setattr(server,
# 'connection_test', ...)` used to work here only because _poll_all_ssh_once
# lived in this file too. It now closes over its own module global, so the same
# line would isolate nothing while looking exactly as if it did.
