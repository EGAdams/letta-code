...
def _resolve_receipt_path(date_str, amount_str, receipt_url=None):
    """Find a receipt file for a row. Order: (date, amount) filename match -> receipt_url
    exact path -> receipt_url stem (any extension) -> receipt_url basename anywhere.
    Returns an absolute path under READABLE_DOCS_BASE, or None."""
    import glob as _glob
    by_da, by_stem = _receipt_index()
...
def _expense_receipt_url(cur, date_str, amount_str, vendor_key, description):
    """DB receipt_url for the expense matching a row (same disambiguation as
    recategorize_expense). Returns the receipt_url string, '' if blank, None if no row."""
    cur.execute(
    ...
    if len(rows) == 1:
        ...
        if chosen is None:
            chosen = rows[0]
    return (chosen.get('receipt_url') or '').strip()


def lookup_receipt(date_str, signed_amount, vendor_key, description=''):
    """Find + return a dashboard-servable URL for the receipt of one row."""
    amt = _norm_amount(signed_amount)
    if amt is None:
        return {'ok': False, 'error': f'Bad amount: {signed_amount!r}'}
    fp = _resolve_receipt_path(date_str, amt, None)
    if not fp:
        try:
            with _rol_get_connection() as cnx:
                with cnx.cursor() as cur:
                    ru = _expense_receipt_url(cur, date_str, amt, vendor_key, description)
        except Exception as e:
            return {'ok': False, 'error': f'DB error: {e}'}
        if ru is None:
            return {'ok': False,
                    'error': 'No matching expense in DB for that date/amount (bank-only row).'}
        if not ru:
            return {'ok': False, 'error': 'No receipt on file for this expense.'}
        fp = _resolve_receipt_path(date_str, amt, ru)
        if not fp:
            return {'ok': False,
                    'error': f'Receipt recorded ({ru}) but the file was not found on disk.'}
    return {'ok': True, 'receipt_url': _receipt_url_for_path(fp)}
...
def receipts_present(rows):
    ...
    try:
        with _rol_get_connection() as cnx:
            with cnx.cursor() as cur:
                cur.execute("SELECT expense_date, amount, receipt_url FROM expenses "
                            "WHERE receipt_url IS NOT NULL AND receipt_url<>''")
                for r in cur.fetchall():
                    key = (str(r['expense_date']), str(r['amount']))
                    if key not in url_map:
                        url_map[key] = (r['receipt_url'] or '').strip()
    except Exception:
        url_map = {}
    out = []
    for row in rows or []:
        ...
        present = False
        if amt is not None:
            if date_str and (date_str, amt) in by_da:
                present = True
            else:
                ru = url_map.get((date_str, amt), '')
                if ru and _resolve_receipt_path(date_str, amt, ru):
                    present = True
        out.append(present)
...
_MAZDA_TOOLS  = ['send_message_to_agent_and_wait_for_reply']
...
LETTA_AGENTS = [
    ...
    {'name': 'Cesare',   'id': None},
    {'name': 'Jeri',     'id': None},
    {'name': 'Mazda',    'id': 'agent-070c201a-8d6d-49ba-a5fd-1489884b3b45', 'required_tools': _MAZDA_TOOLS},
    {'name': 'Mazda Router',           'id': 'agent-81874131-77b8-4645-8d5f-c0bf20c1ed89', 'required_tools': _MINION_TOOLS},
    {'name': 'Mazda Parser',           'id': 'agent-c414cb08-8bb5-4002-8d67-2ad0fbce4f06', 'required_tools': _MINION_TOOLS},
    {'name': 'Mazda Vendor Identity',  'id': 'agent-c1fa4bb4-93a8-4b54-bd54-18fdaa4c0cd6', 'required_tools': _MINION_TOOLS},
    {'name': 'Mazda Receipt Linker',   'id': 'agent-bfffa859-98bf-4bec-af3e-231b552fec93', 'required_tools': _MINION_TOOLS},
    {'name': 'Mazda Categorization',   'id': 'agent-c4c9864c-9046-41b8-b0e3-f97698307567', 'required_tools': _MINION_TOOLS},
]
