...
from intake_report_model import (
    META_EMPTY,
...
def _source_document_reference(chosen, report_path=''):
    """Resolve only distinct, viewable source evidence for a row.

    A scanner report may have a report-directory fallback that points at the
    same paper JPG already stored in ``scanned_statement_url``. That image
    belongs exclusively to the scanned-statement slot; never expose it again
    as View Source Document. A distinct downloaded PDF remains available.
    """
    chosen = chosen or {}
    stored_reference = str(chosen.get('document_url') or '').strip()
    ...
    scanned_reference = str(
        chosen.get('scanned_statement_url') or '').strip()
    source_reference = stored_reference
...
def open_supporting_document(date_str, signed_amount, vendor_key, document_type,
    ...
    if not _viewable_supporting_document(reference, path):
        return {'ok': False,
                'error': f'The {document_type.replace("_", " ")} reference is not a viewable PDF, image, or Excel workbook.'}
    prepared = (
        _prepare_supporting_document_view(chosen, path, document_type)
        if wait_for_highlight
        else _background_annotation_result(chosen, path, document_type)
    )
...
def _supporting_document_view_for_expense(
    ...
    if not chosen or not source_path:
        return None
    result = _background_annotation_result(
