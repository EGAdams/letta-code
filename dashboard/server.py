...
def _block_dispatch_for_human_only_mode(document_path, conversation_id, dispatched_at):
    ...
    just routed to manual processing instead of Mazda's LLM turn.
    """
...
def _dispatch_mazda_or_block(document_path, label, facade, conversation_id,
    ...
    (mazda_dispatched, trainer_dispatched).
    """
    ...
    return True, trainer_dispatched
    ...
    if os.path.isfile(image_path):
        ...
        if remote_image_path:
            ...
            if conversation_id:
                ...
                mazda_dispatched, trainer_dispatched = _dispatch_mazda_or_block(
...
SERVERS = [
    ...
    },
    {
        ...
        'note': 'Browser automation server for relay_message_to_chatgpt tool — controls '
                'a logged-in ChatGPT browser session (:5001). RED = not running or Chrome not '
                'logged into chatgpt.com. Start from relay-messages-to-chatgpt skill.',
    },
    {
        'key': 'mcp-proxy',
...
def start_browser_server():
    """Start browser_server.py if it's not already running.
    Requires: Flask, undetected-chromedriver, Chrome logged into chatgpt.com"""
    _log_restart('browser-server: starting browser_server.py on port 5001')
    browser_server_script = os.path.expanduser('~/letta-code/browser_tools/browser_server.py')
    if not os.path.exists(browser_server_script):
        return {'ok': False, 'text': f'browser_server.py not found at {browser_server_script}'}

    try:
        # Check if already running
        urllib.request.urlopen('http://127.0.0.1:5001/health', timeout=2)
        return {'ok': True, 'text': 'browser_server already running on port 5001'}
    ...
    try:
        ...
        urllib.request.urlopen('http://127.0.0.1:5001/health', timeout=5)
        return {'ok': True, 'text': 'browser_server started on port 5001 (requires Chrome logged into chatgpt.com)'}
    except Exception as e:
        return {'ok': False, 'text': f'failed to start browser_server: {e}'}
