...
export class ManualEntryForm {
  ...
  async mount() {
    ...
    const prefillButton = this._button(shell, "Prefill from OCR", "prefill");
    prefillButton.addEventListener("click", () => this._prefill());
  ...
  async _showArchiveVerification() {
    ...
    try {
      const data = await this.http.postJSON("/api/scanner-archive-path", {
        scanner: this.scannerKey,
      });
      if (!data.ok || !data.archive_path) return;
      this._archiveTerminalEl.innerHTML = "";
      this._archiveTerminalEl.appendChild(
        this._el("h3", { text: `Archive Verification (${data.archive_path})` }),
      );
      ...
      if (session && session.sendLine) {
        session.sendLine(
          buildArchiveVerifyCommand(data.archive_path, data.archive_name || ""),
        );
      }
    } catch {
