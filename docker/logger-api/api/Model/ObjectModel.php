...
class ObjectModel extends Database /* implements ObjectInserter, ObjectUpdater, ObjectDeleter, ObjectSelector */ {
	...
	public function selectObject( $object_view_id ) {
        return $this->select(
            "SELECT * FROM $this->table WHERE object_view_id=?",
            [ "s", [ $object_view_id ] ]
        ); }

    public function deleteObject( $object_view_id ) { return $this->delete(
        "DELETE FROM $this->table WHERE object_view_id=?",
        [ "s", [ $object_view_id ] ]); }

    public function insertObject( $object_view_id, $object_data ) {
        try {
            $this->insert(
                "INSERT INTO $this->table ( object_view_id, object_data ) VALUES ( ?, ? )",
                [ "ss", [ $object_view_id, $object_data ] ]
            );
    ...
    public function updateObject( $object_view_id, $object_data ) {
        return $this->update(
            "UPDATE $this->table SET object_data=? WHERE object_view_id=?",
            [ "ss", [ $object_data, $object_view_id ] ] ); }
}
