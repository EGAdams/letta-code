...
async function main() {
  ...
  try {
    ...
    await log(
      `OPENAI_API_KEY: ${auth.OPENAI_API_KEY === null ? "null (correct)" : String(auth.OPENAI_API_KEY ?? "(missing)")}`,
    );
    const lastRefresh = auth.last_refresh;
    await log(
      `last_refresh: ${lastRefresh ? String(lastRefresh) : "(missing)"}`,
    );
    const hasAccessToken =
      typeof auth.access_token === "string" && auth.access_token.length > 0;
    const hasIdToken =
      typeof auth.id_token === "string" && auth.id_token.length > 0;
    await log(`access_token present: ${hasAccessToken}`);
    await log(`id_token present: ${hasIdToken}`);
  } catch (err) {
    await log(
      `~/.codex/auth.json ERROR: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (providers.length === 0) {
        ...
        for (const p of providers) {
          await log(
            `  provider: ${p.provider_name ?? p.name} type=${p.provider_type}`,
          );
        }
        const hasChatgpt = providers.some(
          (p) =>
            p.provider_name === "chatgpt-plus-pro" ||
            p.provider_type === "chatgpt_oauth",
        );
        await log(
          `chatgpt-plus-pro registered: ${hasChatgpt ? "YES" : "NO — run /connect chatgpt"}`,
        );
      }
    } else {
    ...
    await log(
      `/v1/providers ERROR: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (models.length === 0) {
        await log(
          "ERROR: Server lists zero models — provider not registered or server misconfigured",
        );
      } else {
        const providers = [
          ...new Set(
            models.map((m) => String(m.provider_name ?? m.provider ?? "")),
          ),
        ];
    ...
    await log(
      `/v1/models ERROR: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  if (envKey) {
    await log(
      `WARNING: OPENAI_API_KEY is set in env (${envKey.slice(0, 8)}...) — remove from ~/.bashrc`,
    );
  } else {
