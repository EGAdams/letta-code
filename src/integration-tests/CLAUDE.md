...
| `scissari-tool-parity.integration.test.ts` | 3 tests: (1) detects missing/legacy tools immediately — Scissari had only `web_search`+`fetch_webpage` for days undetected (2026-05-27); (2) verifies system-prompt PATCH doesn't strip tools; (3) verifies reconcile doesn't rewrite tool set — requires `LETTA_RUN_SCISSARI_TEST=1` |
| `scissari-web-tool-response.integration.test.ts` | 2 tests: (1) verifies Scissari can call `web_fetch_exa` (Exa MCP) and return a user-visible response — catches the 2026-05-27 stall-error regression where lettabot returned "(I ran into an issue completing that request)" for every web research query; (2) health-check confirming Exa MCP tool is registered and attached to Scissari — requires `LETTA_RUN_SCISSARI_TEST=1` |
