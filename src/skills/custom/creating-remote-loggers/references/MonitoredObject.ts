import type ILogObject from "../abstract/ILogObject";
...
export default class MonitoredObject {
  ...
  constructor(config: {
    ...
    this.logObjects = [
      this.logObjectFactory.createLogObject("constructing...", this),
    ];
    if (
      config.data_source_location?.length === 0 &&
      document.querySelector(".data-source-location")
    ) {
      config.data_source_location =
        document.querySelector(".data-source-location")?.innerHTML || "";
    }
    this.model = new Model(
      new SourceData({
        Runner: FetchRunner,
        url: config.data_source_location!,
      }),
    );
    this.monitorLed = new MonitorLed();
    this.stringifier = new Stringifier();
    const data_config = {
      object_view_id: this.object_view_id,
      object_data: this.stringifier.stringify(this, 3, null, 2),
    };
  ...
  processQueryResult(_event: any, results: { data: string | any[] }) {
    if (results.data.length > 0) {
      console.log(results.data);
    }
  }
  getObjectViewId() {
    return this.object_view_id;
  }
}
