...
read -r server_host server_port < <(
  extract_host_port "${LETTA_BASE_URL}"
)

# Verify that the Letta server is reachable.
...
# Add --agent automatically when LETTA_AGENT_ID is set,
# unless the caller already supplied --agent.
AGENT_ARGS=()
...
# -y is ALWAYS supplied to Letta.
echo "[run-local-in-dir] exec: ${LETTA_BIN} -y ${AGENT_ARGS[*]} $*"
